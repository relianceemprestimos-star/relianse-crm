...
async def _goto_gov_sp_admin_login(connector: PortalSecundarioLegacyConnector, login_url: str, timeout_ms: int) -> None:
    ...
    if (
        (await connector.page.locator("#username, input[name='username']").count() and await connector.page.locator("#password, input[name='senha'], input[type='password']").count())
        or (
            await connector.page.locator("#txtCPF, input[name='cpf']").count()
            and await connector.page.locator("#txtIdentificador, input[name='identificacao']").count()
            and await connector.page.locator("#txtSenha, #password, input[name='senha'], input[type='password']").count()
        )
    ):
        return
    ...
    try:
        await connector.page.locator("#username, input[name='username'], #txtCPF, input[name='cpf']").first.wait_for(state="visible", timeout=8000)
        await connector.page.locator("#password, #txtSenha, input[name='senha'], input[type='password']").first.wait_for(state="visible", timeout=8000)
    except Exception:
        ...
        raise _typed_login_error("LOGIN_FIELDS_NOT_FOUND", "O sistema nao encontrou o formulario de login do Governo de SP.", stage="govsp_admin_tab")
...
async def _fill_gov_sp_admin_credentials(connector: PortalSecundarioLegacyConnector, login: str, password: str) -> dict:
    ...
    identificacao = str((getattr(connector, "credencial", {}) or {}).get("matricula") or (getattr(connector, "credencial", {}) or {}).get("identificacao") or "").strip()
    result = await connector.page.evaluate(
        """({ login, password, identificacao }) => {
          ...
          return {
            ...
            cpfSet: setValue("#txtCPF, input[name='cpf']", login),
            identificacaoSet: setValue("#txtIdentificador, input[name='identificacao']", identificacao),
            passwordSet: setValue("#password, #txtSenha, input[name='senha'], input[type='password']", password),
            usernameValue: document.querySelector("#username, input[name='username']")?.value || "",
            cpfValue: document.querySelector("#txtCPF, input[name='cpf']")?.value || "",
            identificacaoValue: document.querySelector("#txtIdentificador, input[name='identificacao']")?.value || "",
            passwordLength: (document.querySelector("#password, #txtSenha, input[name='senha'], input[type='password']")?.value || "").length,
          };
        }""",
        {"login": login_digits, "password": password, "identificacao": identificacao},
    )
...
def _build_connector(payload: dict, settings) -> PortalSecundarioLegacyConnector:
    ...
    identificacao = str(payload.get("matricula") or payload.get("identificacao") or "").strip()
    credencial = {
        "id": credential_id,
...
async def _open_gov_sp_login_browser(connector: PortalSecundarioLegacyConnector, login: str, password: str) -> None:
    ...
    for attempt in range(1, 7):
        try:
            ...
            if (
                not credential_state.get("passwordLength")
                or (
                    not credential_state.get("usernameValue")
                    and not (credential_state.get("cpfValue") and credential_state.get("identificacaoValue"))
                )
            ):
            ...
            if "login" in post_body and (await connector.page.locator("#username").count() or await connector.page.locator("#txtCPF").count()):
                if attempt < 6:
                    continue
        ...
        if second_stage_detected:
            ...
            if user_already_logged:
                ...
                if not (snapshot.get('successFound') or snapshot.get('operacionalFound') or snapshot.get('consultaMargemFound')):
                    if not getattr(connector, "_already_logged_retry_done", False):
                        setattr(connector, "_already_logged_retry_done", True)
                        print("[LOGIN_FLOW] usuario ja logado desconectado; tentando login novamente", file=sys.stderr, flush=True)
                        try:
                            await connector.page.goto(connector.settings.pdc_portal_url, wait_until="domcontentloaded", timeout=timeout_ms)
                            await connector.page.wait_for_timeout(1200)
                        except Exception:
                            pass
                        return await _open_login_browser(connector, login, password)
                    raise _typed_login_error(
                        'USER_ALREADY_LOGGED_CONFIRM_FAILED',
